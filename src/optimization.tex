...
\transitionFrame{Relating Optimization \& Adversarial Robustness}
...
\begin{frame}{Minimax Framework}
  ...
  \only<3-6>{Optimization Framework's Two Key Parts
    \begin{itemize}
      ...
      \item \onslide<4->{\textbf{\color{red}Inner Maximization}: Find \textbf{worst case} ${\xadv = \X + \perturb}$ with highest loss}
      \item \onslide<5->{\textbf{\color{ForestGreen}Outer Minimization}: Find model parameters~$\params$ that minimize adversarial loss}
    \end{itemize}
  }
  \only<7->{
...
\begin{frame}{So\ldots Are We Done Here?}
  ...
  \vfill
  \onslide<+->{\textbf{\green{Key Contribution}}: Demonstrating adversarial minimax optimization \textit{is} tractable.}
  \begin{itemize}[<+->]
